...
<div id="ctc-root">
  ...
  <div class="ctc-card ctc-input-panel" id="ctc-input-panel">
  ...
  <!-- spacer injected by JS when panel is pinned -->
  <div id="ctc-spacer" style="display:none;"></div>
...
${this._styles()}`);
    ...
    // ── Pin the input panel when user scrolls past it ─────────────────────────
    ...
    _load_structures(company) {
        frappe.call({
            ...
            callback: (r) => {
                ...
                const opts = r.message.map(s => `<option value="${frappe.utils.escape_html(s)}">${frappe.utils.escape_html(s)}</option>`).join('');
    ...
    _render_table(selector, rows, row_fn, total, total_label) {
        const $t = $(selector);
        $t.find('tbody').html((rows||[]).map(r=>`<tr>${row_fn(r).join('')}</tr>`).join('')
            || `<tr><td colspan="3" class="ctc-empty-row">No components</td></tr>`);
        $t.find('tfoot').html(`<tr class="ctc-tfoot-row">
            <td colspan="2"><strong>${frappe.utils.escape_html(total_label)}</strong></td>
            <td class="ctc-right ctc-mono"><strong>₹ ${this._fmt(total)}</strong></td></tr>`);
    }
    ...
    _show_error(msg) {
        let text = msg;
        if (typeof msg==='string' && msg.startsWith('[')) { try { text=JSON.parse(msg).map(m=>m.message||m).join(' '); } catch(e){} }
...
.ctc-kpi-val { font-size:20px; font-weight:700; letter-spacing:-.02em; margin-bottom:4px; }
.ctc-kpi-lbl { font-size:11px; font-weight:500; text-transform:uppercase; letter-spacing:.05em; opacity:.7; }
...
.ctc-table thead th { padding:7px 10px; font-size:10px; font-weight:700; text-transform:uppercase; letter-spacing:.04em; color:var(--text-muted); border-bottom:2px solid var(--border-color); text-align:left; }
...
.ctc-tag { display:inline-block; padding:1px 7px; border-radius:10px; font-size:10px; font-weight:700; text-transform:uppercase; letter-spacing:.04em; margin-left:4px; }
