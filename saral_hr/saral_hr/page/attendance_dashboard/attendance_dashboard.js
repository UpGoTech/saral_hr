frappe.pages["attendance-dashboard"].on_page_load = function (wrapper) {
	...
	const STATUS_META = {
		"Present":         { color:"#22c55e", bg:"#f0fdf4", border:"#bbf7d0", text:"#15803d" },
		"On Tour":         { color:"#0ea5e9", bg:"#f0f9ff", border:"#bae6fd", text:"#0369a1" },
		"Earned Leave":    { color:"#3b82f6", bg:"#eff6ff", border:"#bfdbfe", text:"#1d4ed8" },
		"Casual Leave":    { color:"#8b5cf6", bg:"#f5f3ff", border:"#ddd6fe", text:"#6d28d9" },
		"Comp Off":        { color:"#a855f7", bg:"#faf5ff", border:"#e9d5ff", text:"#7e22ce" },
		"Earned Comp Off": { color:"#d97706", bg:"#fffbeb", border:"#fde68a", text:"#b45309" },
		"Half Day":        { color:"#f59e0b", bg:"#fefce8", border:"#fef08a", text:"#b45309" },
		"Absent":          { color:"#ef4444", bg:"#fef2f2", border:"#fecaca", text:"#b91c1c" },
		"LWP":             { color:"#dc2626", bg:"#fef2f2", border:"#fca5a5", text:"#991b1b" },
		"Holiday":         { color:"#06b6d4", bg:"#ecfeff", border:"#a5f3fc", text:"#0e7490" },
		"Weekly Off":      { color:"#94a3b8", bg:"#f8fafc", border:"#e2e8f0", text:"#475569" },
		"Not Marked":      { color:"#d1d5db", bg:"#f9fafb", border:"#e5e7eb", text:"#9ca3af" },
	};
	...
	if (!document.getElementById("ap-styles")) {
		...
		.ap-card::after {
			...
			width:60px; height:60px; border-radius:50%; background:rgba(255,255,255,.1);
		}
		.ap-card:hover  { transform: translateY(-2px); box-shadow: 0 6px 18px rgba(0,0,0,.14); }
		.ap-card.dim    { opacity: .45; }
		.ap-card.active { transform: translateY(-2px); box-shadow: 0 6px 18px rgba(0,0,0,.22); outline: 2px solid rgba(255,255,255,.6); outline-offset: -2px; }
		.ap-card-icon  { font-size: 18px; margin-bottom: 8px; }
		...
		.ap-chart-leg { display:flex; gap:12px; margin-top:8px; }
		...
		.ap-hol-dot  { width:8px; height:8px; border-radius:50%; background:#06b6d4; flex-shrink:0; }
		...
		.ap-ms-val   { font-size: 36px; font-weight: 900; line-height: 1; }
	...
	function shiftDate(d){
		...
		state.date=dt.toISOString().split("T")[0];
	}
	...
	function renderDaily(d){
		...
		const defs=[
			...
			{icon:"✅",val:d.present,          label:"Present",       bg:"#15803d", f:"present"},
			{icon:"❌",val:d.absent,           label:"Absent/LWP",   bg:"#b91c1c", f:"absent"},
			{icon:"🌴",val:d.on_leave,         label:"On Leave",      bg:"#5b21b6", f:"leave"},
			{icon:"🌗",val:d.half_day,         label:"Half Day",      bg:"#b45309", f:"hd"},
		];
	...
	function renderDailyPanels(d,activeF){
		...
		function buildEmpPanel(title, badgeColor, badgeNum, empList, emptyMsg, emptyColor){
			...
			const $list=$(`<div style="max-height:320px;overflow-y:auto;"></div>`).appendTo($body);
			const draw=(items)=>{
				$list.empty();
				if(!items.length){ $list.html(`<div class="ap-empty">No results.</div>`); return; }
				items.forEach(e=>{
					const ini=initials(e.name), hue=hueFor(e.employee||e.name);
					const statusMeta=e.status?STATUS_META[e.status]||{color:"#94a3b8"}:null;
					$list.append(`<div class="ap-emp-row">
						...
						${statusMeta?`<span style="font-size:11px;font-weight:700;padding:2px 8px;border-radius:5px;background:${statusMeta.bg||"#f8fafc"};color:${statusMeta.color};border:1px solid ${statusMeta.border||"#e2e8f0"};white-space:nowrap;flex-shrink:0;">${e.status}</span>`:""}
					</div>`);
				});
			};
			draw(filtered);
	...
	function renderMonthly(d,hd){
		...
		[{icon:"✅",val:tp,label:"Present Records",bg:"#15803d"},
		 ...
		 {icon:"🌴",val:(ov["Earned Leave"]||0)+(ov["Casual Leave"]||0),label:"Leaves Taken",bg:"#5b21b6"},
		 {icon:"🌗",val:ov["Half Day"]||0,label:"Half Days",bg:"#b45309"},
		 {icon:"🔴",val:ov["LWP"]||0,label:"LWP",bg:"#991b1b"},
		...
		if(depts.length>1){
			...
			$all.on("click",()=>{ buildRows(emps); $ds.find(".ap-dpill").removeClass("active"); $all.addClass("active"); });
			depts.forEach(dept=>{
		...
		$sw.find(".ap-search").on("input",function(){
			const q=$(this).val().toLowerCase();
			buildRows(q?emps.filter(e=>(e.name||"").toLowerCase().includes(q)):emps);
		});
		$main.append($ep);
	}
	...
	function renderYearly(res){
		...
		let tp=0,ta=0,th=0;
		MONTHS.forEach(m=>{ const d=res[m]; if(!d) return; const ov=d.overall||{};
			tp+=["Present","On Tour","Earned Leave","Casual Leave","Comp Off","Earned Comp Off","Half Day"].reduce((s,k)=>s+(ov[k]||0),0);
			ta+=(ov["Absent"]||0)+(ov["LWP"]||0); th+=(ov["Half Day"]||0); });
		const fd=res[MONTHS[0]];
		const $cards=$(`<div class="ap-cards"></div>`).appendTo($main);
		[{icon:"👥",val:fd?fd.total_active:0,label:"Active",bg:"#1e40af"},
		 {icon:"✅",val:tp,label:"Present",bg:"#15803d"},
		 {icon:"❌",val:ta,label:"Absent",bg:"#b91c1c"},
		 {icon:"🌗",val:th,label:"Half Days",bg:"#b45309"},
		].forEach(c=>$(`<div class="ap-card" style="background:${c.bg};">
			<div class="ap-card-icon">${c.icon}</div>
			<div class="ap-card-num" data-t="${c.val}">0</div>
			<div class="ap-card-label">${c.label}</div>
		...
		MONTHS.forEach(m=>{
			...
			const pres=["Present","On Tour","Earned Leave","Casual Leave","Comp Off","Earned Comp Off","Half Day"].reduce((s,k)=>s+(ov[k]||0),0);
			const abs=(ov["Absent"]||0)+(ov["LWP"]||0);
			const lv=(ov["Earned Leave"]||0)+(ov["Casual Leave"]||0)+(ov["LWP"]||0);
			const hol=ov["Holiday"]||0;
			const mp=d?d.total_days*d.total_active:0, pct=mp>0?Math.round((pres/mp)*100):0;
	...
	function openDrawer(employee,empName,month,year){
		...
		$dr.html(`<div class="ap-drawer-hd">
			<div><div class="ap-drawer-title">Loading…</div><div class="ap-drawer-sub">${month} ${year}</div></div>
	...
	function renderChart($container, trend){
		...
		trend.forEach(d=>{
			const ph=Math.round((d.present/mx)*84), ah=Math.round((d.absent/mx)*84);
