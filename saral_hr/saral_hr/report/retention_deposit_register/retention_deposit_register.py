...
def _get_data(f):
    cols = [
        ...
        _col("Date of Joining",          "date_of_joining",  "Data", 190),
        _col("Deduction Upto (3 Yrs)",   "ded_upto",         "Data", 250),
        _col("Retention Amount",         "retention_amount", "Float", 200, precision=2),
    ]
    ...
    rows = frappe.db.sql(
        "SELECT ss.employee AS eid, ss.employee_name, sd.amount AS ret"
    ...
    for row in rows:
        ...
        du  = None
        if doj:
            try: du = add_months(getdate(doj), 36)
            except Exception: pass
        ret = flt(row.ret, 2)
        g  += ret
        sr += 1
...
def _build_html(cols, data, co, mo, yr):
    ...
    cg = "<colgroup>" + "".join(
        '<col style="width:{w};"/>'.format(w=col_pct.get(c["fieldname"], "16%"))
        for c in cols
    ) + "</colgroup>"

    page1_hdr = (
        '<div class="hdr"><div class="co">{co}</div>'
        '<div class="ttl">Retention Deposit Register</div>'
        '<div class="per">For the Month of {mo} {yr}</div></div>'
    ).format(co=co, mo=mo, yr=yr)

    cont_hdr = (
        '<div class="cont-hdr">{co} &mdash; Retention Deposit Register &mdash; {mo} {yr} (contd.)</div>'
    ).format(co=co, mo=mo, yr=yr)
    ...
    def _row_html(row, idx):
        ...
        for c in cols:
            ...
            if fn == "sr_no":
                ...
                html += '<td style="border:{B};text-align:left;background:{bg};{fw}">{v}</td>'.format(B=B, bg=bg, fw=fw, v=str(val) if val else "")
        html += "</tr>"
        return html
...
def print_report(filters):
    ...
    return _save_pdf(_build_html(cols, data, _company_label(filters), filters.get("month",""), filters.get("year","")), "Retention_Deposit_Register")