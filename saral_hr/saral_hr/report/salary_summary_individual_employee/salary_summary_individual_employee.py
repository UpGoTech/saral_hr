...
def _build_html(cols, data, co, mo, yr,
    ...
    def _emp_rows(row, sr, is_grand=False):
        ...
        if not is_grand and eid:
            ...
            if desig:
                extra += (
                    '<tr><td style="border:none;padding:1px 0 0 0;font-size:8.5px;'
                    'color:#555;font-style:italic;line-height:1.3;">{0}</td></tr>'.format(desig)
                )
            if dept:
                extra += (
                    '<tr><td style="border:none;padding:1px 0 0 0;font-size:8px;'
                    'color:#666;line-height:1.3;">{0}</td></tr>'.format(dept)
                )
            emp_inner = (
    ...
    for ab, nm in legend_map:
        if ab not in seen_lgd:
            seen_lgd.add(ab); legend_items.append((ab, nm))
