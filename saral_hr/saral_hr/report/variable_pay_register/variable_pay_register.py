...
def _get_data(f):
    cols = [
        ...
        _col("Employee ID",          "employee_id",            "Data", 185),
        _col("Employee Name",        "employee_name",          "Data", 275),
        _col("Division",             "division",               "Data", 185),
        _col("Monthly Variable Pay", "monthly_variable_pay",   "Float", 170, precision=2),
        _col("Variable Pay %",       "variable_pay_percentage","Percent", 120),
        _col("Variable Pay Amount",  "variable_pay_amount",    "Float", 185, precision=2),
    ]
    ...
    vpa_key = "{yr} - {mo}".format(yr=yr, mo=mo)
    vmap    = {}
...
def _build_html(cols, data, co, mo, yr):
    col_pct = {
        ...
        "employee_name":           "22%",
        "division":                "15%",
        "monthly_variable_pay":    "15%",
        "variable_pay_percentage": "12%",
        "variable_pay_amount":     "18%",
    }
    cg = "<colgroup>" + "".join(
        '<col style="width:{w};"/>'.format(w=col_pct.get(c["fieldname"], "14%"))
        for c in cols
    ) + "</colgroup>"

    page1_hdr = (
        '<div class="hdr"><div class="co">{co}</div>'
        '<div class="ttl">Variable Pay Register</div>'
        '<div class="per">For the Month of {mo} {yr}</div></div>'
    ).format(co=co, mo=mo, yr=yr)

    cont_hdr = (
        '<div class="cont-hdr">{co} &mdash; Variable Pay Register &mdash; {mo} {yr} (contd.)</div>'
    ).format(co=co, mo=mo, yr=yr)
...
def print_report(filters):
    ...
    return _save_pdf(_build_html(cols, data, _company_label(filters), filters.get("month",""), filters.get("year","")), "Variable_Pay_Register")