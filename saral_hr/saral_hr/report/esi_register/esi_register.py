...
_SKIP_ON_TOTAL = {"esic_number", "days_paid"}
...
def _get_data(f):
    cols = [
        ...
        _col("ESI No.",       "esic_number",   "Data", 190),
        _col("Employee ID",   "employee_id",   "Data", 150),
        _col("Employee Name", "employee_name", "Data", 210),
        _col("Days Paid",     "days_paid",     "Float",  85, precision=2),
        _col("Gross Salary",  "gross_salary",  "Float", 125, precision=2),
        _col("Emp ESIC",      "emp_esi",       "Float", 115, precision=2),
        _col("Empr ESIC",     "empr_esi",      "Float", 115, precision=2),
        _col("Total ESI",     "total_esi",     "Float", 115, precision=2),
    ]
    ...
    slips = frappe.db.sql(
        ...
        "       emp.name AS eid, emp.esic_number"
    ...
    def _em(like, pf):
        ...
        for r in frappe.db.sql(
            "SELECT sd.parent AS slip, sd.amount"
            " FROM `tabSalary Details` sd"
            " WHERE sd.parent IN %(sn)s"
            "   AND sd.parentfield=%(pf)s"
            "   AND LOWER(sd.salary_component) LIKE %(lk)s"
            "   AND sd.amount > 0",
...
def _build_html(cols, data, co, mo, yr):
    ...
    cg = "<colgroup>" + "".join(
        '<col style="width:{w};"/>'.format(w=col_pct.get(c["fieldname"], "10%"))
        for c in cols
    ) + "</colgroup>"

    page1_hdr = (
        '<div class="hdr"><div class="co">{co}</div>'
        '<div class="ttl">ESI Register</div>'
        '<div class="per">For the Month of {mo} {yr}</div></div>'
    ).format(co=co, mo=mo, yr=yr)

    cont_hdr = (
        '<div class="cont-hdr">{co} &mdash; ESI Register &mdash; {mo} {yr} (contd.)</div>'
    ).format(co=co, mo=mo, yr=yr)
...
def print_report(filters):
    ...
    return _save_pdf(_build_html(cols, data, _company_label(filters), filters.get("month",""), filters.get("year","")), "ESI_Register")