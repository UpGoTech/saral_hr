...
frappe.ui.form.on('Employee Loan Schedule', {
    ...
    is_deferred(frm, cdt, cdn) {
        ...
        if (row.is_deducted) {
            ...
            frappe.call({
                method: 'saral_hr.saral_hr.doctype.employee_loan.employee_loan.get_submitted_slip_for_month',
...
function undo_deferral(frm, cdt, cdn, emi) {
    ...
    if (row.deferred_to) {
        ...
        if (target_row) {
            const live_target         = locals['Employee Loan Schedule'][target_row.name];
